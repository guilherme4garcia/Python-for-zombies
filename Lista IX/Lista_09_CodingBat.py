...
def same_first_last(nums):
  if len(nums) > 0 and nums[0] == nums[-1]: #não da pra usar >= 1 na lista, foi necessario usar len
    return True
  else:
    return False
...
def maior_ponta(nums):
  ...
  for x in range(len(nums)):
    nums[x] = (maior) #se colocar [] fica com dois colchetes, nessa caso deve ser usado parenteses
  return nums
...
def alarm_clock(day, vacation):
  ...
  if vacation == False:
    if 1 <= day <= 5: #dia da semana, ferias false
      return '7:00'
